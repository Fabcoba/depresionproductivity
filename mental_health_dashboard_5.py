...
with st.sidebar:
    ...
    st.markdown("<p style='font-size:0.9rem;color:#1a2540;font-weight:600;margin-bottom:24px;'>Mental Health and Productivity</p>", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown("<p style='font-size:0.65rem;color:#adb5bd;'>Sources: Our World in Data, UNDP Human Development Index 2017</p>", unsafe_allow_html=True)

dff = countries_df[countries_df["Year"] == sel_year].copy()

# Header (no Tec de Monterrey, no emoji)
st.markdown("""
<div class="dash-header">
  ...
  <div class="lead">
    ...
    and why investing in the first protects the second.
  </div>
</div>
""", unsafe_allow_html=True)

# By the numbers — people icons
  ...
  <div style="display:flex; align-items:center; gap:14px; flex-wrap:wrap;">
    ...
    <div style="margin-left:18px;">
      ...
      <div style="font-size:0.85rem; color:#495057; margin-top:6px;">
        ...
        averaging <b>{avg_dep_global:.1f}%</b> of the population.
      </div>
    </div>
  </div>
  <div style="margin-top:14px; padding-top:14px; border-top:1px solid #e9ecef; font-size:0.82rem; color:#6c757d;">
    Anxiety adds another <b style="color:#1a2540;">{avg_anx_global:.1f}%</b> on top. Many people experience both at once.
  </div>
</div>
""", unsafe_allow_html=True)
...
Darker blue marks countries with heavier depression burden. Hover over any country to read its rate.
...
st.markdown(f"""<div class="section-story">
Ranking countries reveals the gap between the most and least affected populations. The highlighted bar
marks the country carrying the most weight in {sel_year}.
</div>""", unsafe_allow_html=True)
...
Plotting depression rates against the Human Development Index reveals the relationship that defines
this story. Countries with higher depression tend to score lower on human development, a proxy for
productivity, education, and quality of life.
</div>""", unsafe_allow_html=True)

hdi_data = dff[["Code","Entity","Depression (%)"]].merge(hdi_df[["Code","HDI"]], on="Code", how="inner").dropna()
...
st.markdown(f"""<div class="section-story" style="background:#f8f9fa; border-left:3px solid #0096c7; padding:14px 18px; border-radius:0 8px 8px 0; margin-top:0px;">
{interp}
</div>""", unsafe_allow_html=True)

# CHART 4 — Country deep dive
st.markdown('<div class="section-title">04 Explore Any Country</div>', unsafe_allow_html=True)
st.markdown("""<div class="section-story">
Pick a country to see its depression trajectory from 1990 onward, its HDI score, and how it compares against
the global average.
</div>""", unsafe_allow_html=True)
...
The scatter against HDI exposes a relationship that deserves attention: when mental health declines, broader human
development tends to follow. Investing in mental health is not only a humanitarian concern. It is an investment in the
long-term productivity, education, and quality of life of an entire nation.
</div>
""", unsafe_allow_html=True)
