...
fig_map.update_layout(
    title=dict(text=f"Reported Depression Rate by Country ({sel_year})",
               font=dict(size=13, color=TEXT, family="Inter"), x=0.02, y=0.97),
    paper_bgcolor=BG, plot_bgcolor=BG,
    geo=dict(showframe=False, showcoastlines=False, bgcolor=BG, projection_type="natural earth"),
    margin=dict(l=0, r=0, t=60, b=0), height=480,
    font=dict(family="Inter", color=TICK),
    coloraxis_colorbar=dict(title=dict(text="%"), thickness=12, len=0.5, x=0.95, tickfont=dict(color=TICK, size=10))
)
...
fig_map_h.update_layout(
    title=dict(text=f"Healthcare Spending Per Capita by Country ({sel_year})",
               font=dict(size=13, color=TEXT, family="Inter"), x=0.02, y=0.97),
    paper_bgcolor=BG, plot_bgcolor=BG,
    geo=dict(showframe=False, showcoastlines=False, bgcolor=BG, projection_type="natural earth"),
    margin=dict(l=0, r=0, t=60, b=0), height=480,
    font=dict(family="Inter", color=TICK),
    coloraxis_colorbar=dict(title=dict(text="USD PPP", font=dict(color=TICK, size=10)),
                              thickness=12, len=0.5, x=0.95,
...
if len(combined) > 20:
    ...
    # Replace labels with cleaner shorter ones
    clean_labels = [f"Q{i+1}\navg ${row['avg_spending']:,.0f}"
                    for i, (_, row) in enumerate(group_stats.iterrows())]
    bars = ax_bar.bar(clean_labels, group_stats["avg_dep"],
                      color=greens, width=0.55, zorder=2)

    # Value labels above bars
    for bar, val, n in zip(bars, group_stats["avg_dep"], group_stats["n_countries"]):
        ax_bar.text(bar.get_x() + bar.get_width()/2, val + 0.08,
                    f"{val:.2f}%", ha="center", fontsize=12, color=TEXT, fontweight="700")
        ax_bar.text(bar.get_x() + bar.get_width()/2, val/2,
                    f"{n}\ncountries", ha="center", fontsize=8.5,
    ...
    diff_q3_q1 = group_stats[group_stats["SpendingGroup"]=="Q3 Med-High"]["avg_dep"].values[0] - group_stats[group_stats["SpendingGroup"]=="Q1 Low"]["avg_dep"].values[0]
...
if len(combined) > 5:
    ...
    fig_sc.update_layout(
        ...
        xaxis=dict(
            title=dict(text="Healthcare Spending Per Capita (USD PPP, log scale)",
                       font=dict(size=12, color=TEXT)),
            type="log", showgrid=True, gridcolor="#c1c9d2",
            color="#495057", tickfont=dict(size=11, color="#495057")
        ),
        yaxis=dict(
            title=dict(text="Reported Depression (%)",
                       font=dict(size=12, color=TEXT)),
            showgrid=True, gridcolor="#c1c9d2",
            color="#495057", tickfont=dict(size=11, color="#495057"),
            tickformat=".2f", ticksuffix="%"
        ),
        legend=dict(bgcolor="rgba(0,0,0,0)", font=dict(color=TEXT, size=11),
                    x=0.6, y=1.12, orientation="h"),
        height=520, margin=dict(l=20, r=20, t=80, b=40)
    )
...
# Find the latest year of health data available for THIS specific country
country_health_records = health_df[health_df["Code"] == country_code].sort_values("Year")
if len(country_health_records) > 0:
    latest_record = country_health_records.iloc[-1]
    country_health_val = latest_record["HealthSpending"]
    country_health_year = int(latest_record["Year"])
else:
    country_health_val = None
    country_health_year = None
...
with m2:
    if country_health_val:
        st.markdown(f'<div class="kpi-card green"><div class="kpi-label">Health Spending Per Capita</div><div class="kpi-value">${country_health_val:,.0f}</div><div class="kpi-sub">USD PPP ({country_health_year})</div></div>', unsafe_allow_html=True)
    else:
...
# Build a comparison chart: country line + global average + colored gap area
merged_compare = cd[["Year","Depression (%)"]].rename(columns={"Depression (%)":"country"}).merge(
    global_avg.rename(columns={"Depression (%)":"global"}), on="Year", how="inner"
)
merged_compare["gap"] = merged_compare["country"] - merged_compare["global"]
avg_gap = merged_compare["gap"].mean()
direction = "above" if avg_gap > 0 else "below"
gap_color = "rgba(0,150,199,0.18)" if avg_gap > 0 else "rgba(45,134,89,0.18)"
country_color = ACCENT if avg_gap > 0 else GREEN

fig5 = go.Figure()

# Fill area BETWEEN the two lines (the gap)
fig5.add_trace(go.Scatter(
    x=merged_compare["Year"], y=merged_compare["global"],
    mode="lines", line=dict(color="rgba(0,0,0,0)", width=0),
    showlegend=False, hoverinfo="skip"
))
fig5.add_trace(go.Scatter(
    x=merged_compare["Year"], y=merged_compare["country"],
    mode="lines", line=dict(color="rgba(0,0,0,0)", width=0),
    fill="tonexty", fillcolor=gap_color,
    showlegend=False, hoverinfo="skip"
))

# Global average line — thick gray dashed
fig5.add_trace(go.Scatter(
    x=global_avg["Year"], y=global_avg["Depression (%)"],
    name="Global Average", mode="lines",
    line=dict(color="#6c757d", width=2.5, dash="dash"),
    hovertemplate="<b>Global Average</b><br>Year %{x}<br>%{y:.2f}%<extra></extra>"
))

# Country line — bold solid with markers
fig5.add_trace(go.Scatter(
    x=cd["Year"], y=cd["Depression (%)"],
    name=sel_country, mode="lines+markers",
    line=dict(color=country_color, width=3.5),
    marker=dict(color=country_color, size=5, line=dict(color="white", width=1)),
    hovertemplate=f"<b>{sel_country}</b><br>Year %{{x}}<br>%{{y:.2f}}%<extra></extra>"
))

# Add a clear annotation explaining the gap
latest_country = merged_compare.iloc[-1]["country"]
latest_global = merged_compare.iloc[-1]["global"]
latest_year = int(merged_compare.iloc[-1]["Year"])
latest_gap = latest_country - latest_global

fig5.add_annotation(
    x=latest_year, y=(latest_country + latest_global) / 2,
    text=f"<b>Gap: {latest_gap:+.2f}%</b>",
    showarrow=False, xanchor="left", xshift=10,
    font=dict(size=11, color=country_color, family="Inter"),
    bgcolor="white", bordercolor=country_color, borderwidth=1.5, borderpad=6
)

fig5.update_layout(
    title=dict(text=f"{sel_country} vs Global Average — {direction.title()} the Average on Avg",
               font=dict(size=13, color=TEXT)),
    plot_bgcolor=BG, paper_bgcolor=BG,
    font=dict(family="Inter", color="#495057"),
    xaxis=dict(
        title=dict(text="Year", font=dict(size=12, color=TEXT)),
        showgrid=False, color="#495057", tickfont=dict(size=11, color="#495057")
    ),
    yaxis=dict(
        title=dict(text="Reported Depression (%)", font=dict(size=12, color=TEXT)),
        showgrid=True, gridcolor="#c1c9d2", color="#495057",
        tickfont=dict(size=11, color="#495057"),
        tickformat=".2f", ticksuffix="%"
    ),
    legend=dict(orientation="h", y=1.12, font=dict(color=TEXT, size=11)),
    height=430, margin=dict(l=20, r=120, t=70, b=40),
    hovermode="x unified"
)
